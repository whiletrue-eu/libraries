    ...
    public class XmlUrlResolverEx : XmlUrlResolver
    {
        ...
        public override object GetEntity(Uri absoluteUri, string role, Type ofObjectToReturn)
        {
            ...
            switch (absoluteUri.Scheme)
            {
                ...
                case "dynamic":
                    var ResourceName = absoluteUri.Host;
                    if (dynamicContent.ContainsKey(ResourceName))
                        return new MemoryStream(Encoding.Unicode.GetBytes(dynamicContent[ResourceName]));
                    else
                        throw new ArgumentException($"Dynamic content not found: {ResourceName}");
                case "pack":
                    return Package
                        .Open((Stream) GetEntity(PackUriHelper.GetPackageUri(absoluteUri), null, typeof(Stream)))
        ...
        {
            dynamicContent.Add(id.ToLower(), content);
        }
    }
}