...
namespace WhileTrue.Classes.Wpf
{
    ...
    public class TemplatedAdorner : Adorner
    {
        ...
        public TemplatedAdorner(UIElement adornedElement, DataTemplate template, object dataContext)
            : base(adornedElement)
        {
            adornerElement = new ContentPresenter();
            adornerElement.Content = dataContext;
            adornerElement.ContentTemplate = template;
            adornerElement.Visibility = Visibility.Visible;
            adornerElement.SetBinding(WidthProperty,
                new Binding {Source = adornedElement, Path = new PropertyPath("ActualWidth")});
            adornerElement.SetBinding(HeightProperty,
                new Binding {Source = adornedElement, Path = new PropertyPath("ActualHeight")});

            AddVisualChild(adornerElement);
        }
        ...
        {
            adornerElement.Arrange(new Rect(new Point(0, 0), finalSize));
            return new Size(adornerElement.ActualWidth, adornerElement.ActualHeight);
        }
    }
}