...
namespace WhileTrue.Classes.Wpf
{
    ...
    public class CrossThreadCollectionWrapper : DependencyObject, IValueConverter
    {
        ...
        /// </summary>
        public static readonly DependencyProperty EnableItemFadeAnimationsProperty =
            DependencyProperty.RegisterAttached("EnableItemFadeAnimations", typeof(bool),
                typeof(CrossThreadCollectionWrapper), new PropertyMetadata(false, EnableItemFadeAnimationsChanged));
        ...
        public static readonly DependencyProperty FadeInAnimationProperty =
            DependencyProperty.RegisterAttached("FadeInAnimation", typeof(Storyboard),
                typeof(CrossThreadCollectionWrapper));
        ...
        {
            ShareCollectionPerThread = true;
        }

        /// <summary>
        ///     Gets/sets whether one collection is created for each thread that is used as a proxy between the original collection
        ///     and the UI, or one collection is created per control.
        ///     As enabling item animation changes the behaviour of the collectiton (e.g. items are only removed from the
        ///     collection after the fade-out animation played) it may not be desired
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (Collection is IEnumerable && Collection is INotifyCollectionChanged)
                return CollectionWrapper.GetCollectionWrapperInstance((IEnumerable) Collection,
                    ShareCollectionPerThread);
