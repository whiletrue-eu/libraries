...
{
    /// <summary>
    ///     Provides an easy way to specify grouping and sorting for a collection binding using a converter and a formatting
    ///     string as Converter Parameter.
    ...
    public class CollectionViewConverter : IValueConverter
    {
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            else if (parameter is string)
            {
                ...
                var Match = Regex.Match(Parameter.Replace('/', '\n'),
                    @"((\[(?<Group>.*?)\]|(?<SortAsc>.*?)\+|(?<SortDesc>.*?)-)\w*)*");
                while (Match.Success)
                {
                    if (Match.Groups["Group"].Success)
                        ViewSource.GroupDescriptions.Add(
                            new DynamicPropertyGroupDescription(Match.Groups["Group"].Value, ViewSource));
                    else if (Match.Groups["SortAsc"].Success)
                        ViewSource.SortDescriptions.Add(new SortDescription(Match.Groups["SortAsc"].Value,
                            ListSortDirection.Ascending));
                    else if (Match.Groups["SortDesc"].Success)
                        ViewSource.SortDescriptions.Add(new SortDescription(Match.Groups["SortDesc"].Value,
            ...
            {
                throw new ArgumentException("A CollectionViewSource must be given as ConverterParameter",
                    nameof(parameter));
            }
        ...
        private class CollectionSortDescriptionRefreshAdapter
        {
            ...
            private CollectionSortDescriptionRefreshAdapter(
                System.Windows.Data.CollectionViewSource collectionViewSource)
            {
                this.collectionViewSource = collectionViewSource;
                this.collectionViewSource.View.CollectionChanged += View_CollectionChanged;
                this.collectionViewSource.View.SourceCollection.ForEach(AttachItem);
            }
            ...
            {
                e.OldItems?.ForEach(DetachItem);
                e.NewItems?.ForEach(AttachItem);
            }

            private void DetachItem(object item)
            {
                if (item is INotifyPropertyChanged) ((INotifyPropertyChanged) item).PropertyChanged -= PropertyChanged;
            }
            ...
            {
                if (collectionViewSource.SortDescriptions.Any(sort => sort.PropertyName == e.PropertyName))
                    ScheduleRefreshView();
            }
            ...
            private void ScheduleRefreshView()
            {
                ...
                {
                    refreshNeeded = true;
                    collectionViewSource.Dispatcher.BeginInvoke(DispatcherPriority.ContextIdle, (Action) RefreshView);
                }
            }
