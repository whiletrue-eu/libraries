...
namespace WhileTrue.Classes.Utilities
{
    ...
    public static class PrivateAccess
    {
        ...
        public class Handler
        {
            ...
            /// <summary/>
            public FieldType GetField<FieldType>(string fieldName)
            {
                FieldInfo Field = this.sourceType.GetField(fieldName, this.sourceAccessType|BindingFlags.NonPublic);
                
                if( Field == null )
                {
                    throw new MemberAccessException(string.Format("Field '{0}' not found on type '{1}'", fieldName, this.sourceType.FullName));
                }

                return (FieldType)Field.GetValue(this.source);
            }
            ...
            public PropertyType GetProperty<PropertyType>(string propertyName)
            {
                ...
                if (Property == null)
                {
                    throw new MemberAccessException(string.Format("Property '{0}' not found on type '{1}'", propertyName, this.sourceType.FullName));
                }
                if (Property.CanRead == false)
                {
                    throw new MemberAccessException(string.Format("Property '{0}' cannot be read", propertyName));
                }

                return (PropertyType)Property.GetValue(this.source, new object[0]);
            }
