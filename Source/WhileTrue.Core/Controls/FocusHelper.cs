...
namespace WhileTrue.Controls
{
    ...
    public class FocusHelper
    {
        ...
        static FocusHelper()
        {
            InitialKeyboardFocusProperty = DependencyProperty.RegisterAttached(
                "InitialKeyboardFocus",
                typeof (string),
                typeof (FocusHelper),
                new FrameworkPropertyMetadata(
                    null, new PropertyChangedCallback(InitialKeyboardFocusPropertyChanged)
                    )
                );
        }
        ...
        private static void InitialKeyboardFocusPropertyChanged(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                ((FrameworkElement)dependencyObject).AddHandler(FrameworkElement.LoadedEvent, new RoutedEventHandler(WindowLoaded), true);
            }
            else
            {
        ...
        private static void WindowLoaded(object sender, RoutedEventArgs e)
        {
            ...
            string ElementName = GetInitialKeyboardFocus(FrameworkElement);
            ...
            {
                Trace.WriteLine(string.Format("FocusHelper: Element with name '{0}' not found in parent '{1}'.", ElementName, Element));
            }
        }
        ...
        {
            dependencyObject.SetValue(InitialKeyboardFocusProperty, element);
        }
        ...
        {
            return (string) dependencyObject.GetValue(InitialKeyboardFocusProperty);
        }
    }
}