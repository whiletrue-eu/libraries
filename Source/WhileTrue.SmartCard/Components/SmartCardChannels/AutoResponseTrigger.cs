...
namespace WhileTrue.Components.SmartCardChannels
{
    ...
    {
        private readonly Regex ANY_ATOMIC_PI = new Regex(@"\[[^\[\]]*\]", RegexOptions.RightToLeft);
        private readonly string dynamicCommandConstructionExpression;
        private readonly Regex IF_PI = new Regex(@"\[IF:(?<test1>[0-9A-F]{2})==(?<test2>[0-9A-F]{2}),(?<val1>[0-9A-F]{2}),(?<val2>[0-9A-F]{2})\]");
        private readonly Regex MAX_PI = new Regex(@"\[MAX:(?<val1>[0-9A-F]{2}),(?<val2>[0-9A-F]{2})\]");
        private readonly Regex MIN_PI = new Regex(@"\[MIN:(?<val1>[0-9A-F]{2}),(?<val2>[0-9A-F]{2})\]");
        private readonly Regex originalCommandTrigger;
        private readonly Regex originalResponseTrigger;
        private readonly Regex WILDCARD_PI = new Regex(@"\[(?<type>[CR]):(?<name>[A-Za-z][A-Za-z0-9]+)\]");
        ...
        {
            while (this.ANY_ATOMIC_PI.IsMatch(expression))
            {
                MatchCollection Matches = this.ANY_ATOMIC_PI.Matches(expression);
                foreach (Match Match in Matches)
                {
                    string Result = Match.Value;
                    if (this.WILDCARD_PI.IsMatch(Result))
                    {
                        Result = this.ResolveWildcard(this.WILDCARD_PI.Match(Result));
                    }
                    if (this.MIN_PI.IsMatch(Result))
                    {
                        Result = this.ResolveFunctionMin(this.MIN_PI.Match(Result));
                    }
                    if (this.MAX_PI.IsMatch(Result))
                    {
                        Result = this.ResolveFunctionMax(this.MAX_PI.Match(Result));
                    }
                    if (this.IF_PI.IsMatch(Result))
                    {
                        Result = this.ResolveFunctionIf(this.IF_PI.Match(Result));
                    }
