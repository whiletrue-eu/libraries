...
namespace WhileTrue.Components.CardReaderSubsystem.SCP
{
    internal class SCPCommands
    {
        ...
        /// <param name="module_no">0: COM, 4: PCR 320</param>
        /// <param name="address_id">hardware base address</param>
        /// <param name="reset_mode">0: no</param>
        [DllImport("scpclientdll.dll", EntryPoint = "sccp_open", CharSet = CharSet.Auto)]
        private static extern IntPtr sccp_open(int receiver, int module_no, int address_id, int reset_mode);
        ...
        private static extern LowLevelError sccp_init(IntPtr sctp, int receiver, int module_no, int address_id, int reset_mode);
        ...
        /// </summary>
        /// <param name="sctp">pointer to SCTP structure</param>
        /// <param name="application_id">Application number which is defined in the SCAP description</param>
        ...
        private static extern LowLevelError sccp_select_scap(IntPtr sctp, int application_id, out int ret, [In, Out] byte[] rblock, ref int rlen);
        ...
        {
            IntPtr sctp = sccp_open(0x31, 0, /*port*/port, 0);
            if (sctp == IntPtr.Zero)
            {
        ...
        public static void SelectApplicationProtocol(IntPtr sctp, SCAP scap)
        {
            ...
            scapsByHandle[sctp] = Scap;
        }


        public static CardStatus GetStatus(IntPtr sctp)
        {
            byte[] Response = DoIO(sctp, 0xF8, new byte[] {0x04, 0x00}, 0xF0);
            if (Response[0] == 0x00)
            {
        ...
        {
            DoIO(sctp, 0xF8, new byte[] {0x04, 0x02}, 0xF0);
        }

        public static byte[] PowerOn(IntPtr sctp, bool reset)
        {
            if (scapsByHandle.Contains(sctp))
            {
                return ((SCAPCommands) scapsByHandle[sctp]).PowerOn(sctp, reset);
            }
            else
            {
        ...
        public static byte[] SendAPDU(IntPtr sctp, byte[] data)
        {
            if (scapsByHandle.Contains(sctp))
            {
                return ((SCAPCommands) scapsByHandle[sctp]).SendAPDU(sctp, data);
            }
            else
            {
                throw new Exception("No application selected");
        ...
        {
            Size size = InitDisplay(sctp);
            SetCursor(sctp, new Point(size.Width, 0));
        }
        ...
        public static Point GetCursor(IntPtr sctp)
        {
            ...
            byte[] Response = DoIO(sctp, 0xF8, Command, 0xF0);
        ...
        {
            byte[] Response = DoIO(sctp, 0xF6, 0xF0);
        ...
        {
            byte[] Response = DoIO(sctp, 0xF6, new[] {(byte) scap}, 0xF0);
        ...
        {
            byte[] Response = DoIO(sctp, 0xF8, new byte[] {0x01, 0x00}, 0xF0);
            return Response[0];
        }
        ...
        public static bool GetInputHasEnded(IntPtr sctp)
        {
            ...
            byte[] Response = DoIO(sctp, 0xF8, Command, new[] {0xF0, 0xFF});
        ...
        public static byte[] DoIO(IntPtr sctp, int command, byte[] data, int[] excpectedReturnCodes)
        {
            ...
            LowLevelError Error = sccp_io(sctp, command, data, data.Length, ref ReturnCode, ResponseBuffer, ref ResponseLength);
            CheckError(Error);
