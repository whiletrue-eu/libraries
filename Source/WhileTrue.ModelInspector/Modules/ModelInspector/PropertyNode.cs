...
namespace WhileTrue.Modules.ModelInspector
{
    ...
    {
        private static readonly ObjectCache<ObjectCacheKey<INotifyPropertyChanged, PropertyInfo>, INotifyPropertyChanged, PropertyInfo, PropertyNode> nodeCache = new ObjectCache<ObjectCacheKey<INotifyPropertyChanged, PropertyInfo>, INotifyPropertyChanged, PropertyInfo, PropertyNode>((key, value, index) => CreateNode(value, index));

        internal static PropertyNode GetNode(INotifyPropertyChanged value, PropertyInfo info)
        {
            return nodeCache.GetObject(new ObjectCacheKey<INotifyPropertyChanged,PropertyInfo>(value,info), value, info); //objectcachekey also supports null value
        }
        ...
        private void OnValidationChanged(ValidationEventArgs eventargs)
        {
            ...
            {
                this.InvokePropertyChanged(()=>ValidationResults);
            }
        ...
        private void UpdateNode()
        {
            if (this.propertyInfo.GetIndexParameters().Any())
            {
                ...
                this.SetAndInvoke(()=>Value,ref this.value, null);
            }
            else
            {
                object Value;
                ...
                this.SetAndInvoke(()=>this.Value,ref this.value, ModelNodeBase.GetNode(Value));
            }
        }
        ...
        {
            get { return this.owner is IObjectValidation; }
        }

        public ValidationSeverity ValidationSeverity
        {
            get { return this.validationSeverityAdapter.GetValue(); }
        }
